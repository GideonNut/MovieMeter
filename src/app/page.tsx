...
function MovieSection({ id, title, description, contract, contractAddress }: { id: number; title: string; description: string; contract: any; contractAddress: string }) {
  return (
    <div className="flex flex-col items-center">
      ...
      {/* Voting Buttons (Now Separate from the Card) */}
...
function MovieCard({ id, title, description, contract }: { id: number; title: string; description: string; contract: any }) {
  // Read the current votes for the movie
  ...
  // Listen for the Voted event and auto-update UI
  useContractEvents({
    contract,
    events: [votedEvent],
    onLogs(logs) {
      logs.forEach((log) => {
        if (log.args.movieId === id) {
          console.log(`New vote detected for movie ${id}`);
          refetch(); // Update votes automatically when an event occurs
        }
      });
    },
  });
    ...
    <div className="flex flex-col border border-zinc-800 p-4 rounded-lg hover:bg-zinc-900 transition-colors hover:border-zinc-700 w-full text-center">
      <article>
        ...
        <div className="text-sm text-zinc-300 mb-4">
          {isLoading ? "Loading votes..." : `✅ Yes: ${votes?.[0]} | ❌ No: ${votes?.[1]}`}
        </div>
      </article>
    </div>
  );
...
// Separate Voting Buttons Component
function VoteButtons({ id, contract, contractAddress }: { id: number; contract: any; contractAddress: string }) {
